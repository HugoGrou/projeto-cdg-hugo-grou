...
| Variante de dados                          | Numéricas + Engenharia |
...
| :-------- | -------------: | ----------: |
| Cluster 0 |           2086 |      29,62% |
| Cluster 1 |           2298 |      32,63% |
| Cluster 2 |           2659 |      37,75% |

Esta distribuição mostra que os três perfis têm dimensão relevante. Nenhum cluster ficou residual ou demasiado pequeno, o que torna a segmentação mais útil para interpretação e tomada de decisão.

---

### 1.3. Interpretação dos Resultados em Linguagem Simples
...
### 1.4. Imagens Recomendadas para Evidência Visual

As imagens seguintes devem estar guardadas em `reports/figures/`. Como este ficheiro está dentro da pasta `docs/`, o caminho relativo recomendado é `../reports/figures/...`.

#### Silhouette Plot do Modelo Final

![Silhouette Plot do Modelo Final](../reports/figures/m3_silhouette_plot_modelo_final.png)

Este gráfico permite observar a coesão interna dos clusters. A linha vertical representa o valor médio do Coeficiente de Silhueta. O resultado confirma que a solução final apresenta separação aceitável entre os grupos.
...
### 1.5. Caracterização dos Perfis de Clientes
...
| :----------------- | ----------: |
| tenure             |       27,61 |
| MonthlyCharges     |       25,52 |
| TotalCharges       |      689,62 |
| ServiceCount       |        1,25 |
| AvgChargePerTenure |       25,47 |

Este grupo representa clientes com encargos mensais baixos, baixo valor acumulado e poucos serviços subscritos. Também apresenta uma forte presença de clientes sem serviço de internet, de acordo com a caracterização categórica feita no notebook.

Este perfil pode corresponder a clientes com utilização mais básica dos serviços da empresa. São clientes que podem não ter uma relação tão intensiva com a oferta da empresa, mas que podem representar oportunidades de crescimento comercial através de propostas simples e adequadas ao seu perfil.
...
* evitar estratégias demasiado agressivas de venda, pois o perfil indica menor nível de consumo;
...
| :----------------- | ----------: |
| tenure             |       57,29 |
| MonthlyCharges     |       89,79 |
| TotalCharges       |     5127,99 |
| ServiceCount       |        5,56 |
| AvgChargePerTenure |       89,85 |

Este grupo representa os clientes com maior permanência, maior valor mensal, maior valor acumulado e maior número de serviços subscritos. É o segmento com maior intensidade de relação com a empresa.
...
| TotalCharges       |     1065,63 |
| ServiceCount       |        3,12 |
| AvgChargePerTenure |       73,74 |

Este grupo representa clientes com menor tempo de permanência, encargos mensais relativamente elevados e valor total acumulado ainda baixo. Isto é coerente com clientes mais recentes que já têm mensalidades relevantes, mas que ainda não acumularam muito valor ao longo do tempo.
...
* reforçar comunicação nos primeiros meses de contrato;
...
### 1.6. Resposta às Questões de Investigação

#### Questão 1 — Quais são as características demográficas, contratuais, de serviços subscritos e de consumo que melhor descrevem a heterogeneidade dos clientes?

As características que mais contribuíram para diferenciar os clientes foram sobretudo variáveis relacionadas com:

* permanência do cliente: `tenure`;
* valor mensal pago: `MonthlyCharges`;
* valor total acumulado: `TotalCharges`;
* número de serviços subscritos: `ServiceCount`;
* existência de serviço de internet: `HasInternetService`;
* indicador demográfico: `SeniorCitizen`.

Também foram analisadas variáveis categóricas contratuais e de serviços, como `Contract`, `PaymentMethod`, `InternetService`, `OnlineSecurity`, `TechSupport`, `StreamingTV` e `StreamingMovies`, que ajudaram a interpretar os perfis encontrados.

---

#### Questão 2 — Que perfis de clientes podem ser identificados e como se caracterizam estatisticamente?

Foram identificados três perfis:

| Perfil    | Caracterização resumida                                                     |
| :-------- | :-------------------------------------------------------------------------- |
| Cluster 0 | Clientes com baixo consumo, poucos serviços e menor valor económico         |
| Cluster 1 | Clientes antigos, com maior valor acumulado e maior número de serviços      |
| Cluster 2 | Clientes recentes, com mensalidade elevada e valor acumulado ainda reduzido |

Cada perfil foi caracterizado por pelo menos cinco variáveis relevantes: `tenure`, `MonthlyCharges`, `TotalCharges`, `ServiceCount` e `AvgChargePerTenure`. Além disso, foram utilizadas variáveis categóricas para enriquecer a interpretação dos segmentos.

---

#### Questão 3 — De que forma os perfis identificados podem apoiar decisões de gestão comercial e relacionamento com clientes?

Os perfis permitem diferenciar estratégias de relacionamento:

* para clientes de baixo consumo, podem ser usadas campanhas simples de adesão a serviços adicionais;
* para clientes antigos e de elevado valor, devem ser priorizadas ações de fidelização e valorização da relação;
* para clientes recentes com mensalidade elevada, deve existir acompanhamento inicial e comunicação clara sobre os serviços contratados.

Assim, a segmentação permite apoiar decisões mais direcionadas, reduzindo a dependência de campanhas genéricas aplicadas a todos os clientes.

---

### 1.7. Valor para o Utilizador ou Negócio
...
O diagnóstico por cluster mostrou:

| Cluster   | Silhouette média | Percentagem de Silhouette negativa |
| :-------- | ---------------: | ---------------------------------: |
| Cluster 0 |           0,3946 |                              1,53% |
| Cluster 1 |           0,4189 |                              4,44% |
| Cluster 2 |           0,3744 |                              0,00% |
...
### 2.3. Limitação Relacionada com a Escolha dos 3 Clusters
...
### 2.4. Representação Final dos Dados
...
Também deve ser referido que a variável `AvgChargePerTenure` foi criada e usada para interpretação dos perfis, mas apresentou correlação muito elevada com `MonthlyCharges`. Por esse motivo, a sua utilização direta na modelação final deve ser interpretada com cautela, para evitar redundância entre atributos.

---

### 2.5. Contextos de Falha
...
### 3.2. Risco de Discriminação ou Tratamento Desigual
...
Em contexto real, antes de usar esta solução para decisões comerciais, seria necessário validar os segmentos com dados atuais, avaliar impacto sobre diferentes grupos de clientes e garantir conformidade com regras de proteção de dados.

---

## 4. Roadmap e Trabalhos Futuros

### 4.1. Melhoria Técnica
...
### 4.2. Novas Variáveis a Recolher

Para melhorar a utilidade de negócio da segmentação, seria relevante recolher novas variáveis, como:

1. **Satisfação do cliente:** pontuação de satisfação, NPS ou avaliações de atendimento.
2. **Histórico de contactos:** número de chamadas, reclamações ou pedidos ao suporte.
3. **Histórico comercial:** campanhas recebidas, promoções aceites e alterações contratuais.
4. **Informação temporal:** evolução mensal dos encargos, serviços adicionados ou removidos.
5. **Dados geográficos agregados:** região ou zona de serviço, se recolhidos de forma compatível com privacidade.
6. **Indicadores de utilização real:** volume de dados, chamadas, streaming ou outros consumos efetivos.
...
### 4.4. Validação com Conhecimento de Negócio
