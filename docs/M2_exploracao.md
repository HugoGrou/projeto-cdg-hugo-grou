...
Assim, em vez de analisar a distribuição de uma variável-alvo, a análise exploratória incidiu sobre variáveis críticas para diferenciar clientes. Foram consideradas especialmente relevantes as variáveis numéricas `tenure`, `MonthlyCharges` e `TotalCharges`.
...
Foram criados histogramas e boxplots para estas variáveis, permitindo observar a distribuição dos valores, a dispersão e a eventual existência de valores extremos.
...
A variável `TotalCharges` apresentou o maior coeficiente de variação entre as três variáveis numéricas analisadas, o que indica uma dispersão relativa elevada. Esta dispersão é coerente com a natureza da variável, uma vez que o valor total acumulado depende simultaneamente do tempo de permanência (`tenure`) e do valor mensal pago (`MonthlyCharges`).
...
Foram também analisadas as frequências absolutas e relativas das variáveis categóricas. A variável `customerID` foi excluída por ser apenas um identificador. A variável `Churn` não foi utilizada como alvo, mantendo a coerência com a abordagem não supervisionada.
...
Esta análise é importante porque o tipo de contrato, os serviços contratados e o método de pagamento podem contribuir para a formação de perfis diferentes de clientes.
